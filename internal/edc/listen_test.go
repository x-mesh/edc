...
// lsofFieldsFixture는 실제 macOS 호스트의 `lsof -nP -i -FpcnPT` 출력 형태다. 주소만 문서용 대역으로
// 바꿨다. LISTEN, ESTABLISHED, UDP 대기, 연결된 UDP(QUIC), 포트가 없는 *:*, 이름에 공백이 있는
// 프로세스를 모두 담는다.
const lsofFieldsFixture = `p947
crapportd
f12
PTCP
n*:50317
...
// ssRowsFixture는 실제 Ubuntu 호스트의 `ss -Htulnp` 출력이다. -t와 -u를 함께 주면 맨 앞에 프로토콜
// 열이 생긴다. %iface 표기와 IPv6 [::]도 그대로 담는다.
...
func TestParseLsofFieldsKeepsOnlyWaitingSockets(t *testing.T) {
	sockets, unparsed := parseLsofFields(lsofFieldsFixture, false)
	if unparsed != 0 {
...
func TestParseLsofFieldsExcludesConnectedUDP(t *testing.T) {
	sockets, unparsed := parseLsofFields(lsofFieldsFixture, true)
	if unparsed != 0 {
		t.Fatalf("unparsed = %d, want 0", unparsed)
	}
...
func TestParseSSRowsReadsProtocolColumn(t *testing.T) {
	sockets, unparsed := parseSSRows(ssRowsFixture, true)
	if unparsed != 0 {
		t.Fatalf("unparsed = %d, want 0", unparsed)
	}
	...
	// udp를 끄면 tcp만 남는다.
	tcpOnly, _ := parseSSRows(ssRowsFixture, false)
...
func TestDedupeListenSocketsMergesDualStack(t *testing.T) {
	sockets, _ := parseSSRows(ssRowsFixture, true)
